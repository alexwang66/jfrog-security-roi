...
const T = {
  ...
  zh: {
    ...
    // Feature 4: Cost of inaction
    costOfInaction:     (cost, hrs)  => `⚠ 当前无Curation年度风险敞口：${cost}（${hrs}小时）`,
    // Feature 5: Combined summary
    combinedSummaryBox: (totalInv, totalBenefit, net, roi, payback) =>
...
function renderCombinedReport(input, curation, jas) {
  ...
  document.getElementById("combined-kpi-investment").textContent = CURRENCY.format(totalInvestment);
  document.getElementById("combined-kpi-benefit").textContent    = CURRENCY.format(totalBenefit);
  document.getElementById("combined-kpi-savings").textContent    = CURRENCY.format(combinedNet);
  document.getElementById("combined-kpi-roi").textContent        = `${DECIMAL_2.format(combinedROI)}x`;
  document.getElementById("combined-kpi-payback").textContent    = formatMonths(combinedPayback);
  document.getElementById("combined-kpi-threeyear").textContent  = CURRENCY.format(threeYear);
  ...
  return { combinedNet, combinedROI, combinedPayback };
}
